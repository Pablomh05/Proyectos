...
public class DirectoryConnector {
	public static final String MESSAGE_TO_SEND = "ping";
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) {
	    ...
	    try {
	        ...
	        System.err.println("Error al configurar el tiempo de espera del socket: " + e.getMessage());
	        return null;
		}
		...
		while(attempts < MAX_NUMBER_OF_ATTEMPTS) {
			...
			try {
				if(attempts>1) {
					System.out.println("Forwarding datagram, attempt number "+ attempts + " of " + MAX_NUMBER_OF_ATTEMPTS);					
				}
	            ...
	            if(attempts==MAX_NUMBER_OF_ATTEMPTS) {
		            System.err.println("Attempt limit exceeded.");
	            }
	        } catch (IOException e) {
	            System.err.println("Error de I/O al enviar/recibir datagrama: " + e.getMessage());
	            System.exit(-1);
	        }
		}
	...
	public boolean testSendAndReceive() {
		...
		byte[] requestData = MESSAGE_TO_SEND.getBytes();
		System.out.println("Sending datagram with message \"" + MESSAGE_TO_SEND + "\"");
		System.out.println("Destination is Directory at addr: " + directoryAddress);
        byte[] responseData = sendAndReceiveDatagrams(requestData);

        // Verificar que la respuesta no es nula y empieza con "pingok"
        if (responseData != null) {
            String responseString = new String(responseData);
            if (responseString.startsWith(MESSAGE_TO_SEND+"ok")) {
                success = true;
            }
        }
		return success;
	...
	public boolean pingDirectoryRaw() {
		 ...
		 */
		String message = MESSAGE_TO_SEND +"&"+NanoFiles.PROTOCOL_ID;
		byte[] requestData = message.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);

        if (responseData != null) {
            String responseString = new String(responseData);
            if (responseString.startsWith(NFDirectoryServer.RESPONSE_MESSAGE)) {
        		System.out.println("Success!");
                success = true;
            }else {
        		System.err.println("Failure. The received datagram does not have the message \"" + NFDirectoryServer.RESPONSE_MESSAGE + "\"");
            }
        }
		return success;
	}
	...
	public boolean pingDirectory() {
	    ...
	    DirMessage pingMessage = new DirMessage(DirMessageOps.OPERATION_PING, NanoFiles.PROTOCOL_ID);
	   	...
	   	if(responseData != null) {
		   	...
		   	if (responseMessage!=null && responseMessage.getOperation().equals(DirMessageOps.OPERATION_SUCCESSFUL)) {
		   		success = true;
		    }
	   	}
		return success;
	...
	public boolean registerFileServer(int serverPort, FileInfo[] files) {
	    ...
	    DirMessage serveMessage = new DirMessage(DirMessageOps.OPERATION_SERVE, serverPort, fileList);
	   	...
	   	String messageString = serveMessage.toString();
	 ...
	 */
	public Map<FileInfo, List<InetSocketAddress>> getFileList() {
		Map<FileInfo, List<InetSocketAddress>> fileMapServers = new HashMap<FileInfo, List<InetSocketAddress>>();
		// TODO: Ver TODOs en pingDirectory y seguir esquema similar
		DirMessage fileListMessage = new DirMessage(DirMessageOps.OPERATION_FILELIST);
		
		String messageString = fileListMessage.toString();
	   	...
	   	if(responseData != null) {
	   		String responseString = new String(responseData, 0, responseData.length);
		   	DirMessage responseMessage = DirMessage.fromString(responseString);
		   	if (responseMessage!=null && responseMessage.getOperation().equals(DirMessageOps.OPERATION_AVAILABLE_FILES)) {
		   		fileMapServers = responseMessage.getAvailableFiles();
		   	}
	   	}
		return fileMapServers;
	}
	...
	public InetSocketAddress[] getServersSharingThisFile(String filenameSubstring) {
		...
		DirMessage downloadMessage = new DirMessage(DirMessageOps.OPERATION_DOWNLOAD, filenameSubstring, true);
		
		String messageString = downloadMessage.toString();
	   	...
	   	if(responseData != null) {
	   		String responseString = new String(responseData, 0, responseData.length);
		   	DirMessage responseMessage = DirMessage.fromString(responseString);
		   	if (responseMessage != null && responseMessage.getOperation().equals(DirMessageOps.OPERATION_SERVERS_SHARING_THIS_FILE)) {
				List<InetSocketAddress> tempList = new ArrayList<>();
				for (InetSocketAddress server : responseMessage.getServersWithFile().keySet()) {
					tempList.add(server);
				}
				serversList = tempList.toArray(new InetSocketAddress[0]);
			}
	   	}
		return serversList;
	}
	...
	public boolean unregisterFileServer(int serverPort) {
		...
		DirMessage msg = new DirMessage(DirMessageOps.OPERATION_UNREGISTER, serverPort);
