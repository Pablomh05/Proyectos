...
public class FileInfo {
	...
	public static void printToSysoutWithServers(Map<FileInfo, List<InetSocketAddress>> fileMapServers) {
		StringBuffer strBuf = new StringBuffer();
		strBuf.append(String.format("%1$-30s", "Name"));
		strBuf.append(String.format("%1$10s", "Size"));
		strBuf.append(String.format(" %1$-45s", "Hash"));
		strBuf.append(String.format(" %1$-45s", "Servers"));
		System.out.println(strBuf);
		for (FileInfo file : fileMapServers.keySet()) {
			System.out.println(file +" "+ fileMapServers.get(file));
		}
	}
